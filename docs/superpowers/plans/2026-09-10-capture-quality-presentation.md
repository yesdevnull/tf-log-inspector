...
- [x] **Step 5: Run `go test ./internal/model ./internal/attrib -count=1`, then the full suite.** Confirm exact locations and context counters against raw input. Self-review, signed commit `Retain source locations and context evidence`, then independent task review.
...
- [x] **Step 3: Wire model.Load once.** Collect contexts exactly once, correlate using C1 rules, assemble input including both interner overflow counters before locals are discarded, and store quality after those results exist. Keep UISaturatedDurations until its last renderer is migrated in Task4; then remove that redundant field and migrate all tests/callers in the same commit.
- [x] **Step 4: Add lazy reconstruction status with a failing test.** Fresh load/CaptureQuality/ReconstructionQuality must remain not_checked. Calling ProviderResponse runs the existing whole-capture reconstruction once; success becomes checked with the actual response count, failure becomes failed with the fixed code. A checked capture containing no reconstructed responses is not evidence that every entry is a response. Do not change the strict parser's acceptance rules or expose its error text in quality output.
...
- [x] **Step 3: Add an indicator whose wording does not claim overall validity.** Normal header label is `i quality`; if any limitation is present use `i limitations`. Limitations include rejected records, position exclusions, saturation/clamping, parser/schema anomalies, capped tracking, overflow, incomplete contexts and attribution uncertainty/no context. A not_checked reconstruction state alone does not assert corruption; the panel always discloses it. Never print a score, percentage health or “all valid”. Retain the indicator at supported narrow widths before expendable filename text, using the existing header-fitting rules. Preserve explicit saturation warning until the panel/indicator replacement is visible at all existing golden widths; then remove the redundant UISaturatedDurations field and migrate its callers/tests to Quality.UI.DurationLowerBound.
- [x] **Step 4: Render sections in order.** Timing availability and admitted/rejected/positioned counts; per-stage anomaly rows with first source location when available; attribution numerator/denominator/confidence distribution; context limitations; reconstruction status. Label positions as original one-based physical lines. First samples call SourceLocation with the stored ordinal; failed lookup yields `location unavailable`. Include the recognition limitation: malformed hclog headers are not recovered as independent RPC records. The panel does not invent a count of such records.
- [x] **Step 5: Cover modal precedence and lazy status.** While raw/response search is active, i inserts text. While help or response is open it keeps that modal's existing behaviour. After viewing a response, close it and open quality: status reflects checked/failed without triggering another reconstruction. After filtering out every span, quality counts stay identical. Escape leaves filters intact; next Escape follows the existing underlying behaviour. Test Ctrl-C/q, zero height/width handling, Unicode/control-bearing filenames and short/narrow layouts.
- [x] **Step 6: Document and verify visually.** Add i to help and README. Run TUI tests before updating goldens. For intended changes only, run `go test ./internal/tui -update`, inspect via `scripts/read-golden.sh` and review styling diffs. Verify live terminal interaction at existing normal/narrow widths and page through all sections, including first-location text and reconstruction status. Record exact fixture/keys/results.
- [x] **Step 7: Self-review, signed commit `Expose capture quality in the terminal interface`, and independent review.** Confirm the renderer never recomputes scan facts or attribution on keypress.

## Final validation and delivery

- [x] Separate test-cleanup subagent after implementation, preserving all denominator/privacy/location/modal boundaries; review edits and rerun affected tests.
- [x] Verify TUI/profile/diagnose agree on the controlled capture's admission and attribution figures; filters affect selection figures only. Verify ordinary profile does not reconstruct bodies and diagnose still streams input.
- [x] Run `gofmt -d .`, `go mod tidy -diff`, `go mod verify`, `golangci-lint run --timeout=5m`, `go test -race -count=1 ./...`, `go build ./...`. Require pristine output and inspect all intentional goldens.
- [x] Measure model.Load and first/repeated SourceLocation calls with a sanitised large fixture using Go benchmarks. Record allocations and first-call versus repeated-call behaviour. Do not add caching, parallel loading or a storage change without measurement and a separate justified design.
- [x] Whole-branch review, address findings, record execution evidence here and verify signed commits/clean status. Leave merge/push choice to Dan; remote integration requires existing CI.
...
- These were planning results. The implementation and validation record is maintained below.

## Execution evidence — 10 September 2026

- Implemented sequentially after C1 on `wip/capture-evidence`, starting at `4042426`. Signed commits: `359c97c` exact source locations/context facts; `e0941ef` resource-key schema validation; `ca86f6d` immutable quality facts and lazy reconstruction state; `979e630` RPC issue locations; `6aa56be` profile/streaming diagnose integration; `65233ad` shared text presentation; `d3e1b19` terminal panel; `851169b` denominator, uncertainty and disclosure fixes; `051c173` accurate scanner-count units.
- Independent task reviews and separate test-cleanup passes completed for all four tasks and their fixes. Reviews resolved unsupported resource-key shapes, missing RPC first samples, concurrent reconstruction-status coverage, duplicate report rendering, no-context denominators, Likely attribution indicators and source/count-unit disclosures. A proposed lower-bound defect was retracted after both reviewer and implementer verified that every saturated duration is excluded from positioning, making the two lower-bound flags equivalent.
- TDD evidence includes real loaded source locations beyond 65,535 continuation lines, explicit context/schema cases, detached snapshot mutation, concurrent lazy-status readers, writer-backed profile/streaming parity and keyboard/rendered-output regressions. New interfaces initially produced compile failures; later API-independent baseline reproduction confirmed that `i` did not open a panel on `65233ad`. That later baseline check is not claimed as the original test-first chronology. Focused missing positioning totals, escaped location text and review-fix wording assertions also failed before their corrections.
- Actual terminal checks used a sanitised eight-line capture with a real response body, RPC durations 10ms and 20ms, a rejected negative duration, a before-origin timestamp, one completed UI hook and one incomplete context. At 110×34, the panel matched profile/diagnose: three RPC records, two admitted/30ms, one rejected, one positioned/10ms, one excluded/20ms, one UI duration/1000ms, and nameable duration 10/30ms (33.3%). First issue samples mapped to original physical lines 8, 6 and 5.
- `i`, PgDn and Esc reached all sections without reconstructing responses. Searching Raw Log for `HTTP` selected original Entry 3, column 65; `r` displayed the real decoded response. Closing it restored that position, and the quality panel then reported one checked response. At 110×25, hiding every provider span with `f`/Space left all capture facts unchanged; Esc closed quality while retaining the filter, and the next Esc cleared it.
- At 60×15, long issue locations and explanations wrapped without losing text; at 60×9, paging reached reconstruction with close/scroll/page and quit hints visible. The no-context provider fixture showed unavailable attribution over its retained 6ms RPC denominator. All ten intentional header/help goldens were inspected using `scripts/read-golden.sh` and raw styling diffs. Existing panes and timeline output were unchanged. The full indicator is verified at 60/70/100/160 columns; a 20-column terminal clips the label and is outside that verified width set.
- Benchmarks committed as signed `d2d340f` use a generated 7,077,888-byte fixture: 16,384 recognised provider entries and 49,152 original physical lines. On Apple M4/macOS 15.7.7 with Go 1.27.1, `go test ./internal/model -run '^$' -bench 'Benchmark(Load|SourceLocation(First|Repeated))$' -benchmem -benchtime=200ms -count=3` measured Load at 13.74–13.96ms, about 38.74MB and 102,721 allocations; first SourceLocation at 1.924–2.009ms, 1,956,216 bytes and 26 allocations; repeated lookup at 15.40–16.02ns with zero allocations. Fixture generation and Load are excluded from lookup measurements; a fresh uninitialised Log is used for each first lookup without copying a used sync.Once. These are fixture-specific measurements, not performance guarantees; no production optimisation was added.
- Final whole-branch review covered `6c3ae73..d2d340f`, including both plans and the benchmark method. Its remaining diagnostic-guidance and stale-comment findings were resolved in signed `b7ce72b` and `488221d`. Three real CLI cases distinguish rejected RPC durations, observed structured output without usable context, and ordinary provider messages without RPC records. Each preserves observed counts and empty stderr. Scoped re-review found both findings addressed and no new breakage. Separate cleanup retained the benchmark functions/helpers and the final regression boundaries.
- Fresh final checks on `488221d`: `go test -race -count=1 ./...` passed all eleven packages; `golangci-lint run --timeout=5m` (2.13.2) reported zero issues; `go build ./...`, `gofmt -d .` and `go mod tidy -diff` produced no errors or changes; `go mod verify` verified all modules. All twenty-two commits since local main `6c3ae73` have verified signatures. The implementation is complete, with merge and push left to Dan.
