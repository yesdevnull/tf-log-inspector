...
- [x] **Step 2: Measure.** `go test ./internal/model -run '^$' -bench 'BenchmarkResource' -benchmem -benchtime=200ms -count=3` passed on Go 1.27.1, Darwin 24.6.0 arm64, Apple M4. Final median results were BuildResourceIndex 43.02ms, 20,173,152B and 450,098 allocations; unconstrained SelectResources 5.85ms, 12,897,010B and 30,152 allocations; provider only 5.33ms, 10,750,706B and 30,149 allocations; exact resource only 2.50ms, 664B and 13 allocations; module only 17.40ms, 5,855,200B and 180,342 allocations; provider plus exact resource 2.12ms, 664B and 13 allocations; and provider plus module 12.14ms, 3,935,200B and 120,342 allocations. Inspection found no source-byte rescans or collection-size-dependent nested lookup. Module selection structurally parses the selected and observed module paths for each match, which explains its measured allocation cost; D2 owns projection reuse between relevant filter changes, so D1 adds no cache or storage design. D2 Task 4 reran the same result-checked cases three times at `7625e50`: build 44.04–46.97ms, unconstrained 5.91–6.38ms, provider 5.20–5.63ms, resource 2.50–2.53ms, module 17.35–17.54ms, provider/resource 2.12–2.40ms and provider/module 12.06–14.36ms. The model code was unchanged and the consistent results required no additional cache.
- [x] **Step 3: Separate test cleanup.** Independent cleanup retained both benchmark functions, all standalone and combined selection cases, and the pointer-loop correction. Earlier task cleanup retained the parser/selection regressions and removed one redundant index test with unchanged coverage. Task 4 review and scoped rereview approved the final standalone filter measurements in `388b6f0`.
...
- [x] **Step 5: Whole-D1 independent review and signed evidence commit.** Benchmark and verification commits `90450f7` and `388b6f0` are signed. Whole-D1 independent review of `0528049..388b6f0` found no outstanding issue and confirmed the actual API supports D2's planned consumers. Review covered parser-to-attribution metadata, independent observation facts, original indices, disjoint evidence, missing-type priority, unknown membership and unchanged correlation/quality behaviour. The controller reran the full race suite (all 11 packages), build, lint (zero issues), formatting (no diff), module tidy (no diff) and module verification at `388b6f0`; all passed. Every implementation commit's signature was verified as G. D1 is ready for D2; terminal implementation and terminal validation belong to that next plan.

**D1 handoff completion:** D2 consumed the reviewed API without changing D1 accounting or attribution. The completed TUI builds one index per immutable Log and caches selection projections between relevant filter changes. The obsolete TUI `uiFilter` provider translation was removed because D1 owns the approved evidence-specific filter scopes; no compatibility path was added. Whole-boundary review through signed `69d1fb9` approved the combined D1/D2 result with no outstanding finding, and the final D2 race/build/lint/format/module gates passed locally.
