...
- [x] **Step 1: Add integration assertions before updating outputs.** A sanitised accounting workflow now drives real keys through Resources, literal narrowing, exact selection, evidence, quality, Calls and the scoped raw request. It asserts the 35/10/20/5 ms partition, unchanged whole-log quality, exact raw entry 4 (zero-based index 3), line 1, horizontal query anchor, match and one-entry request scope across evidence open/close. A second sanitised fixture proves two repeated UI operations retain original indices 0 and 1 while aggregate Enter remains inert. The same parsed fixture exposed a named attribution with authoritative `Address` and absent optional `Name`; the behavioural RED rendered `(none)`, and the smallest GREEN display fallback now shows that exact address without parsing or changing attribution.
...
- [x] **Step 4: Real terminal validation.** The controller built `/tmp/tfli-resources` from `7625e50` plus the stable Task 4 production correction and exercised the sanitised fixtures in real `zsh -f` PTY 8810. At 100×30 the workflow preserved UI under provider exclusion; showed 35/10/20/5 ms partitions and method-scoped 15/10 ms; kept whole-log 35/1000 ms quality unchanged; displayed the corrected exact Calls address; and preserved raw entry 4, line 1 and column 97 across quality. At 60×15 quoted modules, explicit root and context-only selection behaved correctly. At 60×9 no-UI routes, all nineteen evidence pages, close and quit were reachable. The PTY exited zero with no remaining terminal finding. The snapshot predates the signed Task 4 commit only by the bounded documentation, tests and golden additions; production source is identical to the inspected snapshot.
- [ ] **Step 5: Separate cleanup and final checks.** A separate test-cleanup agent reviews completed tests; independently review edits without reducing behavioural coverage. Run `go test -race -count=1 ./...`, `go build ./...`, `golangci-lint run --timeout=5m`, `gofmt -d .`, `go mod tidy -diff`, `go mod verify`. Preserve existing linux/darwin amd64/arm64 CI build matrix and distinguish local/CI evidence. The controller reran `go test ./internal/model -run '^$' -bench '^BenchmarkResource(BuildIndex|Select)$' -benchmem -count=3` at `7625e50`; every size assertion passed and the measured ranges remained consistent with D1. Module-only selection remained the highest-allocation projection, while same-Log index/projection reuse already owns the appropriate cache boundary, so no new cache was added. Cleanup and final gates remain pending.
