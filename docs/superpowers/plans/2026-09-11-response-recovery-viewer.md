...
- [x] **Step 1: Add the sanitised journey fixture.** Use these physical lines, including a final newline:
...
- [x] **Step 5: Commit verified completion.** Commit `Complete response recovery viewer integration` with a signature. Record actual APIs, JSON decision, terminal evidence, test commands and review results; update the spec to mark Boundary I complete only when true. Integration remains Dan's decision.
...
| --- | --- |
| One parser; independent retained messages and safe diagnostic ranges | `ProviderResponseAt` consumes the unchanged I1 `InspectProviderJSON` outcome; source-entry preservation and detached diagnostic tests pass. |
| Selected physical position, multiple bodies within one entry | Model and TUI broad-entry tests select distinct bodies by line offset; first-displayed-line tests cover scope, filter and invalid top offsets. The real PTY search opens Entry 3's recovered response. |
| Invalid response vs unavailable stream vs ordinary position | Model/TUI tests and both PTYs distinguish invalid line 2, unavailable line 4, ordinary lines 5–6 and incomplete line 7, with safe source facts. |
| Recovered full body plus notice | Complete-body and notice-layout tests pass, including fragment scope and heights 0/1/2/8. PTY decoded search and resizing retain the notice and usable body. |
| Scrolling/search/controls/return/history | Existing modal regressions and the real `Model.Update` journey pass. Both PTYs return to the exact raw occurrence; the independent history expectation detects a pointed-value mutation missed by a shallow copy. |
| Lazy complete/partial/failed quality | Canonical snapshots, atomic first-access tests, frozen report snapshots and quality-panel tests pass. PTYs show not checked before inspection and partial with two responses/two diagnostics afterwards. |
| Profile/comparison contract remains explicit | Both v2 encoder contracts and invalid-snapshot rejection tests pass. Actual CLI stdout decodes as one document per invocation with the exact ordered lazy/null reconstruction fields; published v2 documents match. |
| Strict scrub refusal even after viewer inspection | The same-source journey requires actual `scrub.Scrub` failure and a zero result after inspection. Existing strict-recovery and CLI no-publication regressions pass in the final race suite. |
| Performance, races, source immutability and detachment | Local/global failure-tail benchmarks retain 128 B/op and one allocation through 100,000 tail lines. Source preservation, detached values and concurrent lookup/snapshot tests pass in the full race suite. |
...
Task 2 completed in signed commit `25c39fe`. Independent task review approved specification and quality without findings. Separate cleanup retained all eight new test functions without changes. Behavioural RED demonstrated entry-wide wrong-body selection, missing recovery notices, generic failure status and invalid-top-line selection; focused and full Go suites then passed. No goldens changed. Task 4 supplies the real terminal evidence below.

Task 3 completed in signed commit `f76670d`. Independent review approved specification and quality without findings; separate cleanup retained all 57 classified units across 30 changed test functions. Focused behavioural RED/GREEN, the required four-package suite and the full Go suite passed. Both JSON roots now emit v2 with the shared state/count/null/code contract; existing UTF-8 and output-protection checks remain, with arbitrary reconstruction codes rejected by the stricter v2 validation. Both new schema documents passed local link/fence/example-syntax checks. Task 4 supplies the final platform evidence below.

Task 4 integration landed in signed commit `18b5b84`. The exact sanitised seven-line fixture and one real `Model.Update` journey cover lazy initial quality, Entry 3 raw and decoded search, exact return position and detached history, all seven physical-line selections, partial quality with two responses and two diagnostics, and an actual `scrub.Scrub` error with an exact zero result after inspection. The journey passed on its first focused run against Tasks 1–3, so it is regression evidence rather than a production behavioural RED. Independent task review approved specification and quality with two minor follow-ups: the expected history now comes from a separately detached navigation snapshot, and the temporary task report is no longer tracked. A temporary `raw.match.entry` pointer mutation reproduced the old shallow snapshot's false negative and made the strengthened exact-history assertion fail; an earlier `raw.query` mutation was insufficient because strings are copied by value. Separate cleanup retained all 22 logical journey cases without edits.

Asserted real PTY journeys passed at 100×30 and 60×9. Both opened lazy quality before inspection, searched Raw Log to Entry 3, opened and searched the decoded recovered body, kept the persistent partial notice visible and decoded content available through resize in both directions, returned to the exact raw occurrence, showed distinct safe states for invalid line 2, unavailable line 4, ordinary lines 5–6 and incomplete line 7, then displayed partial 2/2 quality. The full `go test -race -count=1 ./...` suite passed all eleven packages. Build, formatting, module-tidy, module-verification, lint and Linux/macOS amd64/arm64 package and CLI cross-build checks passed; lint reported zero issues and format/tidy/build logs were empty. A host CLI emitted one profile v2 document and one comparison v2 document; all three fresh reconstruction snapshots used the exact ordered `state`, `responses`, `diagnostics`, `code` keys with `not_checked` and three null details. These are local validation results, not remote CI.

All fourteen commands in the final validation matrix passed on the application tree delivered in `18b5b84`. The focused journey and full race suite passed again after the final test change in `c30d0fe`; `7ae5fca` corrects only the mutation evidence, with another clean focused pass after removing temporary probes. Separate cleanup retained the strengthened history case. Both scoped re-reviews are clean. No production code changed after the matrix, and no golden files changed on this branch.

The controller and independent reviewer inspected the sanitised terminal transcripts in `/tmp/tfli-i2-terminal` and full validation logs in `/tmp/tfli-i2-validation`. The initial PTY harness grouped logical keys into single events and produced mismatched screens; that evidence was rejected. Corrected runs separate key events, wait for rendered readiness and assert each expected screen and return footer before reporting success. The actual host CLI profile and comparison use `testdata/provider-rpc.log` and `testdata/two-rpcs.log`; their saved JSON is under `/tmp/tfli-i2-validation/cli-json`.

The final independent whole-branch review of `04d28da..7ae5fca` found no Critical, Important or Minor issues. It checked the approved contracts, implementation, tests, public documentation, signed history and retained validation evidence. All plan and implementation findings are resolved, with no parked findings or rulings. I1 and I2 now satisfy Boundary I. All branch commits have verified signatures; integration into main remains Dan's decision.
